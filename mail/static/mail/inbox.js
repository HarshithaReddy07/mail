        ...
        .then(email => {
            ...
            ["subject", "timestamp", "sender", "recipients", "body"].forEach(email_element => {
                ...
                if (email_element === "subject") {
                    ...
                    (origin_mailbox === "sent" ?
                        data_buttons_add.slice(0,1) : data_buttons_add)
                    .forEach( text_function => {
                        const text = text_function[0];
                        const callback_function = text_function[1];
            ...
            back_button.append(back_button_col_div);
            document.querySelector("#single-email-back-section").append(back_button);
...
function load_mailbox(mailbox) {
  ...
  fetch(`/emails/${mailbox}`)
    .then(response => response.json())
    .then(emails => {
        const sections_show = [['sender', 5], ['subject', 3], ['timestamp', 4]];
